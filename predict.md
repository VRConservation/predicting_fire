...
El niño? In past big El Nino recent events temperatures have soared globally 2 degrees C or more beyond the norm, although some forecasts say this year the increase could be as much as 3 degrees [@nyt].
...
# Predictive mapping
Generally, when examining wildfire probability or history datasets for California, it can be hard to distinguish the dominant oranges or reds from anything else. {numref}`perimeters` shows all wildfires greater than 5,000 acres (2,023 hectares) since 2010 [@frap]. Again, the map mostly looks orange, but if we zoom in on key areas where forests occur, there are some spots where there haven't been large fires since 2010, such as northwestern Shasta County, Nevada, and Sierra Counties.
